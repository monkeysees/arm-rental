...
`--users 4 --mode virtual` is diagnostic only; `--users 500,1000 --mode all
...
eight requested 500-recipient results, four supplemental completed Go stress
results, the disk-full failure and the deliberately interrupted launcher. The
...
The final requested validation covers 500 recipients: one virtual gate and three
wall runs for each native runtime. The user stopped further 1,000-recipient
execution during the Rust 500-recipient series. Already completed Go stress
results remain supplementary evidence; no Rust 1,000-recipient result or
cross-runtime stress conclusion is claimed for this boundary.

The initial Go protocol exhausted the host filesystem during its final stress
repeat because completed synthetic databases accumulated. Its failure log and
manifest are retained. The coordinator now removes each database **after** final
sampling, independent verification and container removal. Seven earlier valid
Go results were retained, and the failed stress repeat was rerun once. Native
binaries, source, fixture contents, rates, limits, holder and measured service
behavior were unchanged. The outside-service report also gained mode-consistency
validation and clearer process-error diagnostics. Exact initial coordinator and
report sources are retained under `initial-source/`; hashes distinguish the two
harness revisions instead of claiming one frozen revision across the interruption.
