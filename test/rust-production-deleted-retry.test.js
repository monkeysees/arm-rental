...
test(
  "deleting a retry-delayed recipient lets the crawl finish and another recipient continue",
  ...
  async (t) => {
    ...
    const server = createServer(async (request, response) => {
      ...
      if (method === "getUpdates") {
        await delay(30);
        result = [];
        if (retryAt && !deletionSent) {
          deletionSent = true;
          const chat = { id: 42, type: "private" };
          const from = { id: 42 };
          result = [
            { update_id: 1, message: { chat, from, text: "/delete_my_data" } },
            {
              update_id: 2,
              callback_query: {
                id: "delete",
    ...
    await waitFor(() => retryAt && deletionSent && secondApartmentPageAt);
    ...
    assert.equal(
      ...
      3,
      "only the failed delivery, deletion prompt, and receipt should reach the deleted user",
    );
    assert.ok(
      calls
        .filter((call) => call.chat_id === 42)
        .every((call) => !call.text.includes("/ru/item/")),
      "no listing should be sent after deletion",
    );
    assert.ok(
      calls.some(
