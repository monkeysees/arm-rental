...
| Recovery acceptance                   | `ops/production-exercise` records allowlisted runtime source/access/redelivery verification, restore, failed-deployment rollback, quarantine skip, Docker restart, host reboot, and timer-freshness outcomes. Tests prove pass/fail/pending handling and prevent raw journal, user IDs, apartment data, or secret capture. | Harness verified locally; every VPS exercise is pending          |
...
| Timers                            | Version-controlled persistent UTC timers own deploy, monitor, storage, backup, image cleanup, maintenance, restore drill, and reboot checks; monitor data includes last/next/result.                                    | All eight production timers enabled, triggered, successful, and inside their freshness limits. A newly installed never-triggered timer remains pending.                                  |
